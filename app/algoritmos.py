...
class Algoritmos:
    ...
    def get_saldo_al_corte(self, id: int = None) -> pd.DataFrame:
        """
        Obtiene una muestra de los datos de cuentas_octubre para verificar la estructura y calcular el SALDO AL CORTE.

        Args:
            id (int, optional): Si se especifica, filtra por ese ID.

        Returns:
            pd.DataFrame: Muestra de los datos de la tabla cuentas_octubre.
        """
        filter_clause = "WHERE id = :id" if id is not None else ""
        params = {'id': id} if id is not None else {}
    ...
    def get_interes_a_favor(self, id: int = None) -> pd.DataFrame:
        ...
        WITH datos_promociones AS (
            ...
            {filter_clause}
            GROUP BY 
                p.id, p.monto_original, p.tasa_interes, m.fecha_transaccion,
                c.fecha_corte, c.saldo_inicial
        )
        SELECT 
            id,
            fecha_transaccion,
